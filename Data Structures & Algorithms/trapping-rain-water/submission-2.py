class Solution:
    def trap(self, height: List[int]) -> int:
        ...
        # gets the trapped water in the list
        # uses global var of lastPeakIndex to track the index of the last peak
        def findTrappedWater(heights: List[int]) -> List[int]:
            waters: List[int] = []
            currentWater: int = 0
            lastPeakIndex: int = 0
        ...
        # get all the water up to the main peak
        waters = findTrappedWater(height)

        # trim the list of heights to just the tallest peak and after
        postPeakHeights = height[self.lastPeakIndex:]

        # reverse the list and run our search again
        # getting water level from the peak to the end
        waters.extend (findTrappedWater(postPeakHeights[::-1]))
        return sum(waters)
